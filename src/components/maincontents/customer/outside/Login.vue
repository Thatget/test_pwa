        ...
        <div class="container">
          ...
          <div>
            ...
            <div class="mb-3">
              ...
              <input v-model="password" v-bind:type="passwordFieldType" class="form-control">
            </div>
            <div class="mb-3">
              <input @change="showPassword" type="checkbox" name="" id="" style="margin: 2px 5px 0 0;">
  ...
  async setup () {
    ...
    const logIn = async () => {
      ...
      await axios.post(api).then(async (response) => {
        ...
        await cookies.set('customer_token', response.data, '1h')
        router.go('home')
      })
        .catch(error => {
    ...
    return {
      email, password, logIn, showPassword, passwordFieldType
    }
  }
}
</script>
